...
def plot_eigenvals_paper(results,speedrange,ss_file1,ss_file2,ss_file3,plot_type):
    ...
    if   plot_type == "nominal":
        ...
        lab1_plnt  = np.empty_like(speed_points)
        lab2_plnt  = np.empty_like(speed_points)
        sigma_ref  = np.empty_like(speed_points)
        omega_ref  = np.empty_like(speed_points)
        lab1_ref  = np.empty_like(speed_points)
        lab2_ref  = np.empty_like(speed_points)
        for i, speed in enumerate(speed_points):
            with open(f'drift_sim_data\\no_drift_eigen_data_{speed}mps','rb') as inf:
                tmp = dill.load(inf)
            sigma_plnt[i], omega_plnt[i], lab1_plnt[i], lab2_plnt[i] = extract_eigenvals(tmp["plant"][0],{"lean_rate":tmp["plant"][1]},(-2, 5, 0, 0, 0, 0, -0.5, -6),speed)
            sigma_ref[i],  omega_ref[i], lab1_ref[i], lab2_ref[i]  = extract_eigenvals(tmp["ref"][0],{"lean_rate":tmp["ref"][1]},(-2, 5, 0, 0, 0, 0, -0.5, -6),speed)
...
if(PHASE == "calculate_eig"):
    ...
    for name,style,filter_type,data in experiments:
        ...
        lab1 = np.empty((len(data),))
        lab2 = np.empty((len(data),))
