...
def plot_eigenvals_paper(results,plant_ref_file,friction_file,plant_type,start,stop,step):
    ...
    speed_ax_plant_mtr, eig_theory_plant_mtr = get_eigen_vs_speed(plant_ref_file,'plant',speedrange,SIL_PARAMETERS,cmd2trq_gain=0.9)
    speed_ax_ref_mtr, eig_theory_ref_mtr = get_eigen_vs_speed(plant_ref_file,'ref',speedrange,SIL_PARAMETERS,cmd2trq_gain=0.9)
    ...
    for type, axs in ax.items():
        ...
        # axs.scatter(speed_ax_ref_speed, eig_theory_ref_speed[type],s=4, label="Corrected Speed Reference")
        # axs.scatter(speed_ax_plant_mtr, eig_theory_plant_mtr[type],s=4, label="Corrected Motor Plant")
        # axs.scatter(speed_ax_ref_mtr, eig_theory_ref_mtr[type],s=4, label="Corrected Motor Reference")
