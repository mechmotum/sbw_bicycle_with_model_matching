'''
___[ steer_torque_current_meas_callibration.py ]___
At some point I wanted measure the applied steer torque
by measuring the current going through the front motor.
(This did not work eventually, as the current measured 
was only the applied current and not the total current)
This script gives the relation between the measured 
signal comming from the motor driver, the commanded torque
and actual measured torque.
The data comes from a calibration test where I fixed the 
steer on one side with a lashing strap. On the other side 
I had hanging weight scale. I gave a torque command, measured
the weight on the scale and measured the 'current' from 
the motor driver. (The value of the current is given by 
the moter driver with a voltage between -4 and +4 V)
'''

# NOTE: for this file to work it has to be in the same folder as data_parsing.py
