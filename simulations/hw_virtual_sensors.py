...
def calc_omega(par, bike_states):
    d_psi = par["vel"]*tan(bike_states["delta"])/par["wheelbase"]
    omega_x = bike_states["d_phi"]
    omega_y = sin(bike_states["phi"])*d_psi
    omega_z = cos(bike_states["phi"])*d_psi
...
def calc_enc_count(delta):
    ...
    if(delta < 0): 
        encoder_h = (delta + 2*pi) * HW_IN_LOOP_PARS["rad2ecn_ticks"]
        encoder_f = -delta * HW_IN_LOOP_PARS["rad2ecn_ticks"]
    else:
        encoder_h = delta * HW_IN_LOOP_PARS["rad2ecn_ticks"]
        encoder_f = -(delta - 2*pi) * HW_IN_LOOP_PARS["rad2ecn_ticks"]
